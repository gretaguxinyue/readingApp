...
- **听**:底栏「朗读」,或长按选中后「从这儿读」。
- **自动划屏**:底栏「自动」,页面匀速往上走。播放条上的 －/＋ 随时调速,正文上轻点一下暂停/继续。
- **目录**:顶部「目录」,支持两级。
...
底栏「自动」开始匀速滚动。播放条:暂停 · －/＋ 调速 · 停。速度也能在「Aa → 自动划屏速度」里预设,会记住。

速度的单位是**行/分钟**,不是像素/秒。这样换字号或行距的时候不用重调——同样是 24 行/分,18px 和 24px 下的阅读节奏是一样的。播放条上会实时算出「约 N 秒一屏」,那个数字比行数更容易对上自己的手感。默认 24 行/分,大致对应 450~500 字/分。
...
声音和语速在「Aa → 朗读声音」里选,会记住。语音包用的是系统的:
...
桌面上的快捷键:空格播放/暂停(都没开时是翻页)、`A` 自动划屏、`+` `-` 调速、← → 朗读上下一句、`R` 朗读、`T` 目录、`N` 笔记、`Esc` 返回、`Cmd/Ctrl+O` 导入。
...
Netlify → Add new site → Import an existing project → 选这个仓库,Publish directory 填 `app`,Build command 留空。之后 `git push` 就自动上线。

要是继续手拖:把 **`app` 文件夹**(不是外层)拖到 Netlify 的 Deploys 页面。
