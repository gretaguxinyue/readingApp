...
const rateLabel = r => (Number.isInteger(r) ? r : r.toFixed(1)) + '×';
function setRate(r){
  TTS.rate = r; kvSet('ttsRate', r); paintRate();
  if (TTS.on && !TTS.paused) sayCurrent();
}
function paintRate(){
  $('#ttsRate').textContent = rateLabel(TTS.rate);
  const seg = $('#rateSeg');
  if (!seg.children.length) RATES.forEach(r => {
    const b = document.createElement('button');
    b.textContent = rateLabel(r); b.dataset.r = r;
    b.onclick = () => setRate(r);
    seg.appendChild(b);
  });
  [...seg.children].forEach(b => b.classList.toggle('on', +b.dataset.r === TTS.rate));
}
...
$('#ttsRate').onclick = () => setRate(RATES[(RATES.indexOf(TTS.rate) + 1) % RATES.length]);
/* 从屏幕顶部那句开始读,而不是从整章开头 */
...
/* 滑杆 0–100 → 4–90 行/分。用幂曲线是因为常用的慢速段挤在低位,
   线性映射的话前 20% 的行程就把好用的区间走完了。 */
const lpmOf  = v => Math.round(8 + 92 * Math.pow(v / 100, 1.8));
const sliderOf = l => Math.round(100 * Math.pow(Math.max(0, l - 8) / 92, 1 / 1.8));
function asSpeed(d){ asSetSlider(Math.min(100, Math.max(0, sliderOf(AS.lpm) + d * 4))); }
function asSetSlider(v){
  AS.lpm = lpmOf(v); kvSet('asLpm', AS.lpm); paintAuto();
}
function paintAuto(){
  const on = AS.on;
  $('#autoBar').hidden = !on;
  ...
  const v = sliderOf(AS.lpm);
  if (+$('#asSlider').value !== v) $('#asSlider').value = v;
  if (+$('#asSlider2').value !== v) $('#asSlider2').value = v;
  if (on) { $('#topBar').classList.remove('hide'); $('#botBar').classList.remove('hide'); }
}
$('#autoBtn').onclick = () => AS.on ? asStop() : asStart();
$('#asToggle').onclick = asToggle;
$('#asStop').onclick = asStop;
['#asSlider', '#asSlider2'].forEach(sel =>
  $(sel).addEventListener('input', e => asSetSlider(+e.target.value)));
...
(async () => {
  ...
  const saved = await kvGet('ttsRate', 1);
  TTS.rate = RATES.reduce((a, b) => Math.abs(b - saved) < Math.abs(a - saved) ? b : a);
